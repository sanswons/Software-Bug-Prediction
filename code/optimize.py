...
def adam(params, grads, lr, schedule, t_total, b1=0.9, b2=0.999, e=1e-8, l2=0, vector_l2=False, max_grad_norm=-1, **kwargs):
    ...
    """
    t = tfe.Variable(0, dtype=tf.float32, trainable=False)
    tt = t+1
    updates = [t.assign(tt)]
    if max_grad_norm > 0:
    ...
    for p, g in zip(params, grads):
        if p is None or g is None:
            ...
            m = tfe.Variable(p*0, dtype=tf.float32, trainable=False)
            v = tfe.Variable(p*0, dtype=tf.float32, trainable=False)
            lrt = lr*tfe.sqrt(1-b2**tt)/(1-b1**tt)
